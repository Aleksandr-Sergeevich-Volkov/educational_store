...
class Cart(object):
    ...
    def __iter__(self):
        ...
        for item_key, item in self.cart.items():
            ...
            if product_id in product_dict:
                ...
                item["item_key"] = (
                    item_key  # Сохраняем ключ для использования в шаблонах
                )
                yield item
    ...
    def delivery(self):
        if (
            self.get_total_price() <= self.cost
            and self.__len__() != 0
            and self.session.get("delivery_cost") is not None
        ):
            return self.delivery_cost
        elif (
            self.__len__() != 0
            and self.session.get("delivery_type") == "home"
            and self.session.get("delivery_cost") is not None
        ):
            return self.delivery_cost
        return Decimal("0")
