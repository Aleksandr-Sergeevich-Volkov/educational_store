...
class HomePage(CityContextMixin, TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
            ...
            try:
                ...
                if city_name:
                    ...
                    current_city = City.objects.filter(
                        Q(name__iexact=city_name) | Q(region__icontains=region_name)
                    ).first()
        ...
        context["prod_count"] = Product.objects.filter(available=True).aggregate(
            Count("id")
        )
        ...
        context["popular_brands"] = Brend.objects.exclude(name="Venoteks")[:6]
        return context
        ...
        if query:
            ...
            products = (
                base_query.annotate(
                    ...
                    + TrigramSimilarity("articul", query) * 0.8
                    + TrigramSimilarity("brand__name", query) * 0.7
                )
                .filter(
...
def autocomplete(request):
    ...
    try:
        ...
        products = (
            Product.objects.filter(Q(name__icontains=term) | Q(articul__icontains=term))
        ...
        )  # Только 10 результатов для скорости
        suggestions = []
...
def detail_view(request, pk):
    ...
    return render(
        request, "detail.html", {"object": object, "form": form, "comments": comments}
    )
...
def get_measurement_fields_config(brand):
    ...
    fields_config = {
        ...
        "calf_circumference": {"verbose_name": "Обхват икры", "show_in_results": True},
        "mid_thigh_circumference": {
            "verbose_name": "Обхват середины бедра",
            "show_in_results": bool(
                getattr(sample_size, "mid_thigh_circumference", None)
            ),
        },
        "circumference_under_knee": {
            "verbose_name": "Обхват под коленом",
            "show_in_results": bool(
                getattr(sample_size, "circumference_under_knee", None)
            ),
        },
        "Upper_thigh_circumference": {
            "verbose_name": "Обхват бедра верхний",
            "show_in_results": bool(
                getattr(sample_size, "Upper_thigh_circumference", None)
            ),
        },
    }
...
def size_finder(request):
    ...
    if request.method == "POST":
        if brand_form.is_valid():
            ...
            measurement_form = SmartMeasurementForm(request.POST, brand=selected_brand)
...
def detect_city(request):
    ...
    try:
        ...
        if city_name:
            ...
            return JsonResponse(
                {"success": False, "message": "Не удалось определить город по IP"}
            )
