    ...
    for item in cart:
        ...
        if not color:
            ...
            color = Color.objects.get_or_create(
                name="Не указан", defaults={"code": "#CCCCCC"}
            )[0]
...
def _prepare_initial_form_data(request):
    ...
    initial_data = {"address": "-", "postal_code": "-", "city": "-", "address_pvz": "-"}
...
@staff_member_required
def print_order_torg12(request, order_id):
    """Представление для печати ТОРГ-12"""
    order = get_object_or_404(Order, id=order_id)
